...
export class UsersController {
  ...
  async addToWatchlist(@Request() req, @Body() dto: UpdateWatchlistDto) {
    return this.usersService.addToWatchlist(req.user.id, dto.filmId);
  }
  ...
  async removeFromWatchlist(@Request() req, @Param('filmId') filmId: string) {
    return this.usersService.removeFromWatchlist(req.user.id, parseInt(filmId));
  }
  ...
  async addToLikes(@Request() req, @Body() dto: UpdateWatchlistDto) {
    return this.usersService.addToLikes(req.user.id, dto.filmId);
  }
  ...
  async removeFromLikes(@Request() req, @Param('filmId') filmId: string) {
    return this.usersService.removeFromLikes(req.user.id, parseInt(filmId));
  }
}
